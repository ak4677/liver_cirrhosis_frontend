...
export default function Admindes() {
    ...
    useEffect(() => {
        ...
        else if (localStorage.getItem('fetch') === 'doctor') {
            getdoctors();
        }
        else if (localStorage.getItem('fetch') === 'lab_assistant') {
            getlabassistant();
        }
        else if (localStorage.getItem('fetch') === 'patient') {
            getpatients();
        }
    }, [fetchTarget])
    ...
    return (
        <>
            ...
            {localStorage.getItem('fetch') === 'doctor' && isEditVisible && <AddDoctor className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center" visible={isEditVisible} onClose={hideEdit} refresh={getdoctors}/>}
            {localStorage.getItem('fetch') === 'patient' && isEditVisible && <AddPatient className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center" visible={isEditVisible} onClose={hideEdit} />}
            {localStorage.getItem('fetch') === 'lab_assistant' && isEditVisible && <AddLab_assistant className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center" visible={isEditVisible} onClose={hideEdit} />}
            <div className='flex mt-16'>
                <aside className='w-64 bg-blue-400 h-screen fixed top-16 p-4 shadow-md'>
                    <ul className='space-y-4 grid justify-items-center text-3xl text-white cursor-pointer'>
                        ...
                        <i className='p-2 hover:bg-blue-200 rounded' onClick={() => setFetchTarget('doctor')}>Doctors</i>
                        <i className='p-2 hover:bg-blue-200 rounded' onClick={() => setFetchTarget('lab_assistant')}>lab_assistands</i>
                        <i className='p-2 hover:bg-blue-200 rounded' onClick={() => setFetchTarget('patient')}>patients</i>
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
                        <span className="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow-sm md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700" onClick={update}>
                            <div className="flex  justify-items-center gap-5 p-4 leading-normal" >
                                <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">new assignment</h5>
                                <i className="fa-solid fa-plus text-white content-center"></i>
                            </div>
                        </span>
                        {Array.isArray(assignments) && assignments.length > 0 ? (
                            assignments.map((assistante) => (
                                <div key={assistante._id}>
                                    <AdmissionCard assistante={assistante} />
                                </div>
                            ))
                        ) : (
                            <p className="flex flex-col text-center bg-transparent">
                                No assignments found.
                            </p>
                        )}
                    </div>
                </div>)}
                {fetchTarget === 'doctor' && (<div className='ml-64 p-6 w-full'>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
                        <span className="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow-sm md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700" onClick={update}>
                            <div className="flex  justify-items-center gap-5 p-4 leading-normal" >
                                <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">new Doctor</h5>
                                <i className="fa-solid fa-plus text-white content-center"></i>
                            </div>
                        </span>
                ...
                {fetchTarget === 'patient' && (<div className='ml-64 p-6 w-full'>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
                        <span className="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow-sm md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700" onClick={update}>
                            <div className="flex  justify-items-center gap-5 p-4 leading-normal" >
                                <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">new Patient</h5>
                                <i className="fa-solid fa-plus text-white content-center"></i>
                            </div>
                        </span>
                ...
                {fetchTarget === 'lab_assistant' && (<div className='ml-64 p-6 w-full'>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
                        <span className="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow-sm md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700" onClick={update}>
                            <div className="flex  justify-items-center gap-5 p-4 leading-normal" >
                                <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">new lab-assistant</h5>
                                <i className="fa-solid fa-plus text-white content-center"></i>
                            </div>
                        </span>
