            ...
            <div className="flex items-center bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                ...
                <div className="flex flex-col justify-between p-4 w-full">
                    ...
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                        Patient: {props.doctor.email}
                    </p>
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                        Number: {props.doctor.Number}
                    </p>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                        <span className={`inline-block h-3 w-3 rounded-full ${props.doctor.isActivated ? "bg-green-500" : "bg-red-500"}`}></span>
                        Status: {props.doctor.isActivated?"Active":"InActive"}
                    </p>

                    {/* Placeholder for future info
                    {props.doctortante.notes && (
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                            Notes: {props.doctortante.notes}
                        </p>
                    )} */}
                </div>
